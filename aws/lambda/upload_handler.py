...
SETTINGS_KEY = "settings/upload_settings.json" # S3 object containing the user-configurable processing settings.
READ_CHUNK_SIZE = 65536 # Number of bytes read from the MJPEG file at a time.
                ...
                if not inside_frame:
                    ...
                    if (previous_byte == 0xFF and current_byte == 0xD9):
                        ...
                        if analyze_frame:
                            ...
                            print("Detected labels:", len(detected_labels))
...
def create_result_document(
    ...
    return {
        ...
        "analyzed_frames": (processing_result["analyzed_frame_count"]),
        "processing_duration_ms": (processing_duration_ms),
        "settings": {"target_species": (settings["target_species"]),
            "analyze_every_nth_frame": (settings["analyze_every_nth_frame"]),
            "minimum_confidence": (settings["minimum_confidence"]),
            "maximum_labels_per_frame": (settings["maximum_labels_per_frame"]),
            "minimum_detection_frames": (settings["minimum_detection_frames"])
        },
        "summary": detection_summary,
        "organized_detections": (organized_detections),
        "frames": (processing_result["analyzed_frames"])
    }


# Saves the final processing result as JSON in S3.
def save_result_json(bucket, result_key, result_document):
    result_bytes = json.dumps(result_document, indent=2).encode("utf-8")
...
# downloads the video, extracts frames, performs recognition,
# organizes the results, stores metadata, and updates DynamoDB.
def process_mjpeg_record(record):
    ...
    result_key = ("results/" + video_name + ".json")
    ...
    try:
        ...
        # Step 1: Extract and analyze MJPEG frames.
        processing_result = (
        ...
        # Step 2: Build a video-level detection summary.
        detection_summary = (
            create_detection_summary(processing_result["analyzed_frames"], settings)
        )

        # Step 3: Copy confirmed detections into species folders.
        organized_detections = (
        ...
        # Step 4: Create and upload the processing report.
        result_document = (
        ...
        save_result_json(bucket, result_key, result_document)
